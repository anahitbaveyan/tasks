...
describe('stock_trades_api_hard', () => {
    ...
    afterEach(async function() {
        this.timeout(10000); // Set timeout to 10 seconds
        await connectionManager.clearDatabase();
    });
    ...
    it('should create a new trade of buy type', async function() {
    ...
    it('should create a new trade of sell type', async function() {
    ...
    it('should return 400 if the payload has an invalid type', async function() {
        const payload = { ...user23_buy_ABX, type: 'test' };
        const response = await chai.request(server).post('/trades').send(payload);
        response.should.have.status(400);
    });

    it('should return 400 if the number of shares exceeds the limit', async function() {
        const payload = { ...user23_buy_ABX, shares: 102 };
        const response = await chai.request(server).post('/trades').send(payload);
        response.should.have.status(400);
    });

    it('should return 400 if the number of shares is below the limit', async function() {
        const payload = { ...user23_buy_ABX, shares: 0 };
        const response = await chai.request(server).post('/trades').send(payload);
        response.should.have.status(400);
    });

    it('should fetch all the trades', async function() {
    ...
    it('should fetch no trades if the type filter value does not exist', async function() {
    ...
    it('should fetch all trades for a user', async function() {
    ...
    it('should fetch no trades if user filter value does not exist', async function() {
        await setup(user25_sell_AAC, user24_sell_AAC, user23_sell_AAC, user23_buy_ABX);
    ...
    it('should fetch all buy trades for a user', async function() {
        const trades = await setup(user25_sell_AAC, user24_sell_AAC, user23_sell_AAC, user23_buy_ABX);
    ...
    it('should fetch all sell trades for a user', async function() {
        const trades = await setup(user25_sell_AAC, user24_sell_AAC, user23_sell_AAC, user23_buy_ABX);
    ...
    it('should fetch a single trade', async function() {
